...
class FeedSerializerTestCase(TestCase):
    def test_feed(self):
        ...
        expected = {
            ...
            'tags': [],
            'create_date': localtime(feed.create_date)
            .isoformat()
            .replace('+00:00', 'Z'),
        ...
        expected = {
            ...
            'feed': OrderedDict(
                [
                    ...
                    (
                        'create_date',
                        localtime(feed.create_date).isoformat().replace('+00:00', 'Z'),
                    ),
                    (
                        'write_date',
                        localtime(feed.write_date).isoformat().replace('+00:00', 'Z'),
                    ),
                    ('active', True),
                    ('tag_names', 'No Tags'),
                    ('count', 1),
            ...
            'viewed': False,
            'bookmarked': False,
            'create_date': localtime(content.create_date)
            .isoformat()
            .replace('+00:00', 'Z'),
        }
